...
FastHessian::FastHessian(std::vector<OpenSurfInterestPoint> &ipts, 
...
FastHessian::FastHessian(IplImage *img, std::vector<OpenSurfInterestPoint> &ipts, 
						 const int octaves, const int intervals, const int init_sample, 
						 const float thres) 
						 : ipts(ipts), m_det(NULL), i_width(0), i_height(0)
{
...
void FastHessian::interpolateExtremum(int octv, int intvl, int r, int c)
{
	...
	{
		// Create OpenSurfInterestPoint and push onto Ipoints vector
		OpenSurfInterestPoint ipt;
