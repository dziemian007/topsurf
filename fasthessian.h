...
#ifndef _OPENSURF_FASTHESSIANH
#define _OPENSURF_FASTHESSIANH
...
public:
	...
	FastHessian(std::vector<OpenSurfInterestPoint> &ipts, 
	...
	FastHessian(IplImage *img, 
		std::vector<OpenSurfInterestPoint> &ipts, 
		const int octaves = OCTAVES, 
		const int intervals = INTERVALS, 
		const int init_sample = INIT_SAMPLE, 
		const float thres = THRES);
...
private:
	...
	//! Initial sampling step for OpenSurfInterestPoint detection
	int init_sample;
